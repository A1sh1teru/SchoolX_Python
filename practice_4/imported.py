...
if __name__ == "__main__":
    ...
    for i3 in RACE_DATA.values():
        if i3.get('FinishedPlace') == 1:
            first_winnerTime = f'Время: {seconds_conversion(i3.get("FinishedTimeSeconds"))}'
            break
    ...
    for k3 in RACE_DATA.values():
        if k3.get('FinishedPlace') == 2:
            second_winnerTime = f'Время: {seconds_conversion(k3.get("FinishedTimeSeconds"))}'
            break
    ...
    for r3 in RACE_DATA.values():
        if r3.get('FinishedPlace') == 3:
            third_winnerTime = f'Время: {seconds_conversion(r3.get("FinishedTimeSeconds"))}'
            break
